// controllers/history/history.controller.ts
// Fusión: main (base) + getHistoryStats de feature/jorge-backend
import { Request, Response } from 'express';
import { HistoryService }    from '../../services/history/history.service';

export class HistoryController {

  /** GET /api/history — historial completo del usuario */
  ...
  /** GET /api/history/stats — estadísticas del historial (con categoría favorita) */
  static async getUserStats(req: Request, res: Response) {
    try {
      const userId = (req as any).userId;
      const mode   = req.query.mode; // ?mode=aggregate usa pipeline de Jorge

      const result = mode === 'aggregate'
        ? await HistoryService.getHistoryStats(userId)
        : await HistoryService.getUserStats(userId);
  ...
  /** GET /api/history/:id — detalle de una entrada específica */
  static async getHistoryById(req: Request, res: Response) {
    try {
      ...
      res.status(500).json({ success: false, message: error.message || 'Error al obtener el detalle' });
    }
  }
}
